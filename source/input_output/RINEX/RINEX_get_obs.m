...
obs_struct = struct('L1', zeros(nSatTot,1), 'L2', zeros(nSatTot,1), 'L3', zeros(nSatTot,1),...
    'C1', zeros(nSatTot,1), ...
    'P1', zeros(nSatTot,1), 'P2', zeros(nSatTot,1), 'P3', zeros(nSatTot,1),...
    'S1', zeros(nSatTot,1), 'S2', zeros(nSatTot,1), 'S3', zeros(nSatTot,1),...
    'D1', zeros(nSatTot,1), 'D2', zeros(nSatTot,1), 'D3', zeros(nSatTot,1));

obs_tmp = zeros(nSatTot,4);
...
if (~isempty(sat_types)) %RINEX v2.xx
    ...
    for s = 1 : nSat
        ...
        if (~obs_struct.S3(index))
            obs_struct.S3(index) = obs_tmp(index, 3);
        end           
    end
end
