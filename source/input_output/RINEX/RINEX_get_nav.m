...
while (~feof(fid))
    ...
    if (strcmp(sys_id,'G') || strcmp(sys_id,'R') || strcmp(sys_id,'E') || strcmp(sys_id,'C') || strcmp(sys_id,'J') || strcmp(sys_id,'S'))
        ...
        if (strcmp(sys_id,'G'))
            sys_index = first_id_sys(cc.ID_GPS);
        elseif (strcmp(sys_id,'R'))
            sys_index = first_id_sys(cc.ID_GLONASS);
        elseif (strcmp(sys_id,'E'))
            sys_index = first_id_sys(cc.ID_GALILEO);
        elseif (strcmp(sys_id,'C'))
            sys_index = first_id_sys(cc.ID_BEIDOU);
        elseif (strcmp(sys_id,'J'))
            sys_index = first_id_sys(cc.ID_QZSS);
    ...
    switch sys_id
        case 'G'
            if (~cc.getGPS.flag_enable), continue, end
        case 'R'
            if (~cc.getGLONASS.flag_enable), continue, end
        case 'E'
            if (~cc.getGalileo.flag_enable), continue, end
        case 'C'
            if (~cc.getBeiDou.flag_enable), continue, end
        case 'J'
            if (~cc.getQZSS.flag_enable), continue, end
        case 'S'
            %if (~cc.getSBAS), continue, end
    end
