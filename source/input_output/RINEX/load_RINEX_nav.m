...
% if Klobuchar ionospheric delay correction is requested but parameters are not available in the navigation file, try to download them
    ...
    function parse_file(only_iono)
        ...
        if (cc.getGLONASS.flag_enable && ~only_iono)
        ...
        if (cc.getGalileo.flag_enable && ~only_iono)
        ...
        if (cc.getBeiDou.flag_enable && ~only_iono)
            if (exist([filename(1:end-1) 'f'],'file'))
                %parse RINEX navigation file (BeiDou)
                if(~only_iono), log.addMessage(sprintf('%s',['Reading RINEX file ' filename(1:end-1) 'f: ... '])); end
                [Eph_C, iono_C] = RINEX_get_nav([filename(1:end-1) 'f'], cc);
                if(~only_iono), log.addStatusOk(); end
            elseif (~flag_mixed)
        ...
        if (cc.getQZSS.flag_enable && ~only_iono)
        ...
        if (cc.getIRNSS.flag_enable && ~only_iono)
            if (exist([filename(1:end-1) 'i'],'file'))
                %parse RINEX navigation file (IRNSS)
                if(~only_iono), log.addMessage(sprintf('%s',['Reading RINEX file ' filename(1:end-1) 'i: ... '])); end
